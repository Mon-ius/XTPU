...
sudo -i
sudo apt-get update && sudo apt-get dist-upgrade -y && sudo apt-get install -y lsof

sudo snap remove --purge  oracle-cloud-agent && sudo snap remove --purge core18

# Remove packages only if they exist
for pkg in $(dpkg-query -Wf '${Package}\n' | grep header) $(dpkg-query -Wf '${Package}\n' | grep -E "^linux.*[0-9]{4}-oracle" | grep -v "$(uname -r)"); do
    sudo apt-get purge -y "$pkg" 2>/dev/null || true
done

# Remove other packages if they exist
sudo apt-get purge -y lxc* lxd* vim* 2>/dev/null || true
...
cd /
echo "Mounting tmpfs..."
mount -t tmpfs -o size=1200m tmpfs mnt || { echo "Failed to mount tmpfs"; exit 1; }

echo "Copying filesystem to RAM (this will take several minutes on 1-core systems)..."
tar --one-file-system --exclude='./swapfile' --exclude='./swap.img' --exclude='./tmp/*' --exclude='./var/tmp/*' --exclude='./var/cache/*' --exclude='./var/lib/snapd/*' --exclude='./snap/*' --exclude='./usr/lib/snapd/*' --exclude='./boot/*' --exclude='./mnt/*' --exclude='./lost+found' -cvpf - . 2>/dev/null | tar -C /mnt -xpf - || { echo "tar failed"; exit 1; }

echo "Filesystem copied. Size in tmpfs:"
du -sh /mnt

USED_KB=$(du -s /mnt | awk '{print $1}')
if [ "$USED_KB" -gt 1200000 ]; then
    echo "ERROR: Filesystem ($USED_KB KB) exceeds tmpfs size (1200MB). Aborting."
    umount /mnt
    exit 1
fi
...
tee /tmp/install_debian.sh > /dev/null << 'INSTALL_SCRIPT'
#!/bin/sh
set -e
